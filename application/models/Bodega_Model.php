...
class Bodega_Model extends CI_Model{
    ...
    function registrarIngreso($ingreso){
        ...
        if($registro_cobol->key == 1){
            ...
            $sql_art   = 'INSERT INTO bodega_central.articulos_ingreso (ari_codigo, ari_nombre, ari_cantidad, ari_unidad_medida, ari_espacio, ari_sucursal_destino, ari_ingreso) VALUES (?,?,?,?,?,?,?)';
            ...
            foreach($ingreso->articulos as $articulo){
                if($articulo->cant_recibida > 0){
                    foreach($articulo->estantes as $estante){
                        $this->db->query($sql_art, array($articulo->codigo, $articulo->nomart, $estante->cantidad, $articulo->umed, $estante->estante, $ingreso->bodega_despacho, $id_ingreso));
    ...
    function listarHistorialIngresosPorFecha($fecha_desde, $fecha_hasta){
        $sql   = "SELECT * FROM bodega_central.ingresos WHERE ing_fecha_ingreso >= ? AND ing_fecha_ingreso <= ? ORDER BY ing_fecha_ingreso DESC";
    ...
    function listarEstantesPorArticulo($codigo){
        $sql   = "SELECT *,
                ...
                LEFT JOIN bodega_central.articulos_salida ON ari_id=ars_articulo_ingreso
                INNER JOIN bodega_central.espacios ON esp_id=ari_espacio
                WHERE ari_codigo=? AND ((ari_cantidad-ars_cantidad) > 0 OR ars_cantidad IS NULL)";
    ...
    function registrarSalida($salida, $id_salida){
        ...
        $sql_salida     = "INSERT INTO bodega_central.salidas (sld_id, sld_fecha_salida, sld_bodega_destino, sld_observacion) VALUES (?,?,?,?)";
        $sql_art_salida = "INSERT INTO bodega_central.articulos_salida (ars_articulo_ingreso, ars_cantidad, ars_salida) VALUES (?,?,?)";

        $this->db->trans_begin();
        $this->db->query($sql_salida, array($id_salida, date('Y-m-d H:i:s'), $salida->bod, $salida->detalle->mensaje_nuevo));

        foreach($salida->detalle->codigos as $articulo){
            $this->db->query($sql_art_salida, array($articulo->articulo_seleccionado, $articulo->cant_salida, $id_salida));
        }
    ...
    function listarHistorialSalidasPorFecha($fecha_desde, $fecha_hasta){
        $sql   = "SELECT * FROM bodega_central.salidas WHERE sld_fecha_salida >= ? AND sld_fecha_salida <= ? ORDER BY sld_fecha_salida DESC";
        $query = $this->db->query($sql, array($fecha_desde, $fecha_hasta));
        return $query->result();
    }
