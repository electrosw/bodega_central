...
class Cobol_Model extends CI_Model{
    ...
    function listarOrdenCompra($orden_compra){
        $url  = "https://intranet.inverluz.cl/cgi-bin/bodega/capebodega.sh?opcion=1&ocompra=".$orden_compra;
        $resp = $this->conexionCurl($url);
        if($resp->status == 200 && $resp->data->Cod == 200){
            $resp->data->fecha_despacho     = '';
            $resp->data->bodega_despacho    = '';
    ...
    function registrarIngreso($ingreso){
        ...
        $url  = "https://intranet.inverluz.cl/cgi-bin/bodega/capebodega.sh?opcion=2&ocompra=$orden_compra&codigos=$codigos&gdesp=$guia_despacho&factura=$factura&glosa=$observacion&trut=$ingreso->rut_transportista&foflete=$ingreso->fecha_transporte&kgflete=$ingreso->peso_transporte&oflete=$ingreso->orden_transporte&usuario=$usuario";
    ...
    function listarOrdenesTraspaso($bodega){
        $url  = "https://intranet.inverluz.cl/cgi-bin/bodega/lisotrasp.sh?bodega=".$bodega;
        $resp = $this->conexionCurl($url);
        if($resp->status == 200){
            $resp = $resp->data;
            $resp->status = 200;
    ...
    function listarOrdenTraspaso($id_orden){
        $url  = "https://intranet.inverluz.cl/cgi-bin/bodega/capebodega.sh?opcion=3&ocompra=".$id_orden;
        $resp = $this->conexionCurl($url);
        if($resp->status == 200){
            $resp = $resp->data;
            $resp->status = 200;
    ...
    function registrarSalida($salida){
        ...
        $url  = "https://intranet.inverluz.cl/cgi-bin/bodega/capebodega.sh?opcion=4&ocompra=$orden_traspaso&codigos=$codigos&glosa=$observacion&usuario=$usuario";
